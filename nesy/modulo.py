...
# Define or own operator to replace the Python native sum
def modulo_operator(ell, n=0):
    return ell[0] % (ell[1] + 1)


def modulo(n: int, dataset: str, seed=None):
    """Returns a dataset for binary modulus"""
    return MNISTOperator(
        dataset_name=dataset,
...
def main(N, batch_size, n_epochs, pretrained_mnist=None):
    ...
    train_set = modulo(N, "train")
    test_set = modulo(N, "test")

    if pretrained_mnist is None:
        name_network = "mnist_modulo"
        network = MNIST_Net()
    else:
    ...
    model = Model("models/modulo.pl", [net])
...
if __name__ == "__main__":
    ...
    if os.path.exists(FILE_PATH / "networks" / "mnist_modulo.pth"):
        pretrained_mnist = "mnist_modulo"
    else:
